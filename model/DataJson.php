...
class DataJson
{
    ...
    function writeFileBarthelADL($name , $type , $score){
        $strDataJson = file_get_contents( '/oldperson/model/barthelADLJson.json');
        $jsonData = json_decode($strDataJson,true);
        $write = [];
        ...
        file_put_contents('/oldperson/model/barthelADLJson.json',$writeJson);
        return true;

    }

    function writeFileChulaADL($name , $type , $score){
        $strDataJson = file_get_contents('/oldperson/model/chulaADLJson.json');
        $jsonData = json_decode($strDataJson,true);
        $write = [];
        foreach ($jsonData as $key => $item){
            $write []= $item;
        ...
        $writeJson = json_encode($write);
        file_put_contents('/oldperson/model/chulaADLJson.json',$writeJson);
        return true;
